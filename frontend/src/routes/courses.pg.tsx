...
function PgCoursesPage() {
  ...
  // Filter registry items based on active widget criteria selection
  const filteredProgrammes = activeFilter 
    ? pgProgrammes.filter(p => p.stream === activeFilter)
    : pgProgrammes;
  ...
  return (
    <>
      ...
      {/* Stream Overview Summary Widgets */}
      <Section className="pb-0">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {[
            { id: "Science", label: "Science Masters", count: "6 Programmes", color: "bg-blue-500/5 text-blue-600 border-blue-500/10 hover:bg-blue-500/10" },
            { id: "Arts", label: "Arts Masters", count: "4 Programmes", color: "bg-amber-500/5 text-amber-600 border-amber-500/10 hover:bg-amber-500/10" },
            { id: "Commerce", label: "Commerce Masters", count: "1 Programme", color: "bg-emerald-500/5 text-emerald-600 border-emerald-500/10 hover:bg-emerald-500/10" },
          ].map((widget, i) => (
            <Reveal key={widget.id} delay={i * 0.05}>
              <button 
                onClick={() => setActiveFilter(activeFilter === widget.id ? null : widget.id)}
                className={`w-full p-4 border rounded-xl text-center shadow-sm bg-card transition-all cursor-pointer block relative ${widget.color} ${
                  activeFilter === widget.id ? "ring-2 ring-primary border-transparent scale-[1.02]" : "opacity-80 hover:opacity-100"
                }`}
              >
                <div className="text-xs uppercase tracking-wider font-bold">{widget.label}</div>
                <div className="text-lg font-black mt-1">{widget.count}</div>
                {activeFilter === widget.id && (
                  <span className="absolute top-1.5 right-2 text-[9px] font-bold tracking-tight bg-background px-1.5 py-0.5 rounded border">
                    Filtering
                  </span>
                )}
              </button>
      ...
      {/* Structured PG Course Listing Table */}
      <Section>
              ...
              <div className="p-5 border-b border-border bg-muted/30 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-bold text-primary text-base">
                    PG Course Registry & Timeline {activeFilter ? `(${activeFilter} Only)` : ""}
                  </h2>
                </div>
                {activeFilter && (
                  <button 
                    onClick={() => setActiveFilter(null)}
                    className="text-xs font-bold text-muted-foreground hover:text-primary transition-colors border px-2 py-1 rounded bg-background"
                  >
                    Clear Filter [x]
                  </button>
                )}
                <span className="text-xs font-semibold px-2.5 py-1 rounded-md bg-primary/5 text-primary border border-primary/10">
                  ...
                  <tbody className="divide-y divide-border text-sm">
                    {filteredProgrammes.map((prog, index) => (
                      <tr key={prog.sno} className="hover:bg-muted/10 transition-colors group">
                        ...
                        {/* Course Name with explicit structural Medium Flag */}
                        <td className="p-4">
                          <div className="font-semibold text-foreground flex items-center gap-2 flex-wrap">
                            <span>{prog.course}</span>
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-muted text-muted-foreground tracking-wide uppercase border border-border/60">
                              {prog.medium}
                            </span>
                          </div>
                        </td>
                        
                        {/* Shift Flag indicator layout wrapper */}
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${
                            prog.shift.includes("Shift-II")
