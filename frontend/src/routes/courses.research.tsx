...
import type { ResearchDept } from "@/lib/content-fns";

const STATIC: ResearchDept[] = [
...
export const Route = createFileRoute("/courses/research")({
  ...
  loader: async (): Promise<ResearchDept[]> => {
    try {
      const data = await getCourses();
      if (data.research?.length) return data.research;
    } catch {}
    return STATIC;
  },
...
function ResearchCoursesPage() {
  const registry = Route.useLoaderData() as ResearchDept[];
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const filtered = selectedCategory ? registry.filter((r) => r.category === selectedCategory) : registry;
          ...
          {FILTER_TABS.map((tab, idx) => (
            <Reveal key={idx} delay={idx * 0.04}>
              <button
                ...
                className={`px-4 py-2 text-xs font-bold rounded-xl border transition-all cursor-pointer ${selectedCategory === tab.id ? "bg-primary border-primary text-primary-foreground shadow-sm scale-105" : "bg-card border-border text-muted-foreground hover:text-primary hover:border-primary/40"}`}
              >
                {tab.label}
              </button>
            </Reveal>
                ...
                <div className="border border-border rounded-2xl bg-card shadow-sm hover:border-gold/30 hover:shadow-elegant transition-all flex flex-col h-full overflow-hidden group">
                  <div className="p-5 bg-muted/30 border-b border-border flex items-start justify-between gap-2">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-gold-deep bg-gold/10 px-2 py-0.5 rounded">{dept.category}</span>
                      <h3 className="text-base font-bold text-primary tracking-tight mt-1.5 group-hover:text-gold-deep transition-colors">Dept. of {dept.department}</h3>
                    </div>
