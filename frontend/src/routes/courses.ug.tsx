...
function UgCoursesPage() {
  ...
  // Filters results if a summary widget card is active
  const filteredProgrammes = activeFilter 
    ? ugProgrammes.filter(p => p.stream === activeFilter)
    : ugProgrammes;
    ...
    <>
      ...
      <Section className="pb-0">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
          {[
            { id: "Science", label: "Science Tracks", count: "7 Programmes", color: "bg-blue-500/5 text-blue-600 border-blue-500/10 hover:bg-blue-500/10" },
            { id: "Arts", label: "Arts Tracks", count: "4 Programmes", color: "bg-amber-500/5 text-amber-600 border-amber-500/10 hover:bg-amber-500/10" },
            { id: "Commerce", label: "Commerce Tracks", count: "1 Programme", color: "bg-emerald-500/5 text-emerald-600 border-emerald-500/10 hover:bg-emerald-500/10" },
            { id: "Management", label: "Management Tracks", count: "1 Programme", color: "bg-purple-500/5 text-purple-600 border-purple-500/10 hover:bg-purple-500/10" },
          ].map((widget, i) => (
      ...
      {/* Structured Courses Listing Table */}
      <Section>
              ...
              <div className="p-5 border-b border-border bg-muted/30 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-bold text-primary text-base">
                    UG Course Registry & Timeline {activeFilter ? `(${activeFilter} Only)` : ""}
                  </h2>
                </div>
                {activeFilter && (
                  <button 
